...
install:
	GOBIN=$(GOPATH)/bin go install $(TOOLS_DIR)/$(NAME)/*.go

# build passed in tool project
build: .pre-build
	GOBIN=$(GOPATH)/bin go build -i -o build/$(NAME) -ldflags ${KIT_VERSION} ./$(TOOLS_DIR)/$(NAME)/
...
dev:
	go run -ldflags ${KIT_VERSION} $(TOOLS_DIR)/$(NAME)/*.go \
	-logtostderr \
...
build-docker-bin:
	GOBIN=$(GOPATH)/bin CGO_ENABLED=0 GOOS=linux go build -i -o build/app -ldflags ${KIT_VERSION_DOCKER} -a -installsuffix cgo ./$(TOOLS_DIR)/$(NAME)/
